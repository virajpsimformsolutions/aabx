...
export async function installApk(apkPath, deviceSerial) {
...
async function runAdbCommand(args) {
    return new Promise((resolve, reject) => {
        ...
        child.on("error", (error) => {
            reject(new Error(`Unable to execute adb. Ensure Android Platform Tools are installed and adb is on PATH. (${error.message})`));
        });
        child.on("close", (code) => {
            if (code !== 0) {
